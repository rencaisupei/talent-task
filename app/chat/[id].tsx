...
export default function ChatDetailScreen() {
  ...
  const handleReport = () => {
    Alert.alert('檢舉這組對話', '請選擇檢舉原因，我們會解密對話紀錄交由安全審核人員複核。', [
      ...REPORT_REASONS.map((reason) => ({
        text: reason,
        onPress: () => reportConversation(conversation.id, reason, displayName),
      })),
  ...
  return (
    ...
    >
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View className={cn('max-w-[82%]', isMine ? 'self-end' : 'self-start')}>
              ...
              >
                <Text className="text-muted text-[10px]">{formatClockTime(item.at)}</Text>
        ...
        ListEmptyComponent={<EmptyState title="開始你們的第一則訊息" />}
      />
